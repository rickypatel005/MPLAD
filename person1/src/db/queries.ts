...
export async function getProjects(params: ProjectQueryParams): Promise<PaginatedResult<ProjectEntity>> {
  ...
  if (params.search) {
    ...
    conditions.push(`(
      p.project_id ILIKE $${paramIdx} OR
      p.project_name ILIKE $${paramIdx} OR
      p.mp_name ILIKE $${paramIdx} OR
      p.state_name ILIKE $${paramIdx} OR
      p.category ILIKE $${paramIdx} OR
      p.ia_name ILIKE $${paramIdx}
    )`);
...
export async function addReviewAction(
  ...
  const proj = await query('SELECT project_id FROM projects WHERE project_id = $1;', [projectId]);
